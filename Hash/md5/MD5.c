...
typedef struct t_hash_state {
    unsigned long h[4];
    unsigned char buf[BLOCK_SIZE];    /** 64 bytes == 512 bits == sixteen 32-bit words **/
    unsigned curlen;            /** Useful message bytes in buf[] (leftmost) **/
    unsigned long long totbits;           /** Total message length in bits **/
} hash_state;
...
static void md5_compress(hash_state * hs)
{
 ...
 int MD5_init(hash_state **mdState)
{
    hash_state *hs;
 ...
 int MD5_destroy (hash_state *mdState)
{
    free(mdState);
    return 0;
}

 int MD5_update(hash_state *hs, const unsigned char *buf, size_t len)
{
    if (NULL == hs || NULL == buf) {
        return ERR_NULL;
    }
...
static int md5_finalize(hash_state *hs, unsigned char *hash /** [DIGEST_SIZE] **/)
{
    unsigned left, i;
 ...
 int MD5_digest(const hash_state *mdState, unsigned char digest[DIGEST_SIZE])
{
    hash_state temp;
 ...
 int MD5_copy(const hash_state *src, hash_state *dst)
{
 ...
 */
 int MD5_pbkdf2_hmac_assist(const hash_state *inner, const hash_state *outer,
                                             const unsigned char first_hmac[DIGEST_SIZE],
                                             unsigned char result[DIGEST_SIZE],
                                             size_t iterations)
{
    hash_state inner_temp, outer_temp;
    size_t i;
    unsigned char last_hmac[DIGEST_SIZE];
