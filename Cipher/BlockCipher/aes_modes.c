...
void test_aes_modes()
{
	...
	// cbc
	printf("cbc mode:\n");
	aes_encrypt_cbc(&stcAES, data, nCipherOrPlain,iv, AES_BLOCK_SIZE,  cipher, nCipherOrPlain);
	...
	aes_decrypt_cbc(&stcAES, cipher, nCipherOrPlain, iv, AES_BLOCK_SIZE, plain, nCipherOrPlain);
	...
	// cfb
	printf("cfb mode:\n");
	memset(cipher, 0, nCipherOrPlain);
	memset(plain, 0, nCipherOrPlain);
	aes_encrypt_cfb(&stcAES, data, nCipherOrPlain, iv, AES_BLOCK_SIZE, cipher, nCipherOrPlain);
	...
	aes_decrypt_cfb(&stcAES, cipher, nCipherOrPlain, iv, AES_BLOCK_SIZE, plain, nCipherOrPlain);

	for (i = 0; i < nCipherOrPlain; i++) {
		printf("%02x", plain[i]);
	}
	printf("\n");
	if (!memcmp(plain, data, 0x10))
