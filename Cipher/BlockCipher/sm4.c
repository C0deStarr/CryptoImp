...
ErrCrypto sm4_init(sm4_ctx* pCtx, uint8_t* pKey, uint32_t nKey)
{
	ErrCrypto errRet = ERR_OK;
	if (!pCtx || !pKey)
	{
	...
	, uint32_t nOut/* >= SM4_BLOCK_SIZE*/)
{
	...
	for (i = 0; i < SM4_ROUNDS; ++i)
	{
		x[i + 4] = F(x[i], x[i + 1], x[i + 2], x[i + 3], pCtx->rk[SM4_ROUNDS - i - 1]);
	}
	u32to8_big(&pOut[0], x[35]);
	u32to8_big(&pOut[4], x[34]);
	u32to8_big(&pOut[8], x[33]);
...
void test_sm4()
{
	...
	sm4_init(&sm4, key, nKey);
	sm4_encrypt(&sm4, data, sizeof(data), cipher, 0x10);
	...
	sm4_decrypt(&sm4, cipher, sizeof(cipher), plain, 0x10);
