...
#define ROTL31(a,k)	((((a) << (k)) | ((a) >> (31 - (k)))) & 0x7FFFFFFF)

#define    LFSRWithInitialisationMode(u)			\
	V = LFSR[0];					\
	ADD31(V, ROTL31(LFSR[0], 8));			\
	ADD31(V, ROTL31(LFSR[4], 20));			\
	ADD31(V, ROTL31(LFSR[10], 21));			\
	ADD31(V, ROTL31(LFSR[13], 17));			\
	ADD31(V, ROTL31(LFSR[15], 15));			\
	ADD31(V, (u));					\
...
)
{
	...
	uint32_t W = 0, W1 = 0, W2 = 0, U = 0, V = 0;
	uint32_t i = 0;
	...
	if ((nKey < ZUC_KEY_SIZE)
		|| (nIV < ZUC_IV_SIZE))
	{
		return ERR_MAX_OFFSET;
	}

	memset(pState, 0, sizeof(ZUC));

	LFSR = pState->LFSR;

	for (i = 0; i < 16; i++) {
		LFSR[i] = MAKEU31(pKey[i], D[i], pIV[i]);
	}
...
void test_zuc()
{
	...
	ZUC ctx = {0};
	uint32_t keystream[3] = {0};
	zuc_init(&ctx
		, key, ZUC_KEY_SIZE
		, iv, ZUC_IV_SIZE);
	zuc_generate_keystream(&ctx, 3, keystream);

	memset(key, 0xff, ZUC_KEY_SIZE);
	memset(iv, 0xff, ZUC_IV_SIZE);
	zuc_init(&ctx
		, key, ZUC_KEY_SIZE
		, iv, ZUC_IV_SIZE);
	zuc_generate_keystream(&ctx, 3, keystream);
